...
begin
  ...
  RLLabelDataAbastecimento.Caption := vQuery.FieldByName('DATA').AsString;
...
procedure TfRelatorios.RLBandHeaderBeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
  RLLabelGrupo.Caption := Trim(VQuery.FieldByName('TANQUE').AsString);
end;

procedure TfRelatorios.RLBandSummary01BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
  RLLabelValorTotalTanque.Caption := 'R$ ' + FormatFloat('#,##0.00',vVlrTotalTanque);
  vVlrTotalTanque := 0;
end;

procedure TfRelatorios.RLBandSummary02BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
  RLLabelValorTotal.Caption := 'R$ ' + FormatFloat('#,##0.00',vVlrTotal);
  vVlrTotal := 0;
end;
